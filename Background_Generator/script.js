...
const css = document.querySelector('h3'); // selecting h3 which will display the code for the colors
const color1 = document.querySelector('.color1'); // ".color1" since you are selecting class="color1"
const color2 = document.querySelector('.color2'); // samedt
const body = document.getElementById('gradient'); // this will be the var so that the bg gradient will be selected
const random = document.getElementById('random'); // this will be the var for the random button
...
function randomGradient() {
  ...
  body.style.background =
    `${'linear-gradient(to right, ' + '#'}${randomColor()}, ` +
    `#${randomColor()})`;
  css.textContent = `${body.style.background};`;
}
